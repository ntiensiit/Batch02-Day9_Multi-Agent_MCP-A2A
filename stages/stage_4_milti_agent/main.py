...
async def check_routing(state: LegalState) -> dict:
    ...
    try:
        ...
        # Fallback to keyword-based routing if LLM returns invalid JSON
        ...
        needs_privacy = any(kw in question_lower for kw in ["data", "privacy", "gdpr", "dữ liệu", "personal"])
        parsed = {"needs_tax": needs_tax, "needs_compliance": needs_compliance, "needs_privacy": needs_privacy}
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a specialist in data protection and privacy law with expertise in GDPR, CCPA, "
        "data breach notification requirements, consent management, and individual privacy rights. "
        "Use the search_compliance_law tool to ground your analysis (as it includes privacy laws). "
